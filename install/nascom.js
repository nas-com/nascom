...
function add_comment(text){
    ...
    var callArgs = JSON.stringify([text, url]);
    nebPay.simulateCall(dappAddress, "0", "add_comment", callArgs, {
        listener: function(resp) {
            var result = JSON.parse(resp.result);

            if (result.success){
                console.log(result);
                nebPay.call(dappAddress, "0", "add_comment", callArgs, {
                    listener: function(resp){
                        render_comment(text);
                    }
                });
...
function render_comment(text, id, author="You", datetime="Just now"){
	var container = $("#nascom");
	var comment = "<div class='nascom-comment'><span class='author'>"+author+"</span><span class='time'>"+datetime+"</span>"+text+"<a href='#' class='delete' style='display:none;' data-comment-id='"+id+"'>x</a></div>";
...
window.addEventListener('message', function(e) {
    try{
        ...
        var callArgs = JSON.stringify([window.location.hostname, nascom_website_id]);
        nebPay.simulateCall(dappAddress, "0", "get_websiteinfo", callArgs, {
            listener: function(resp) {
                var result = JSON.parse(resp.result);
                if(result.admin == user_account){
                    isAdmin = true;
                    setTimeout(function(){
                        $('#nascom a.delete').show();
                            $("#nascom a.delete").on("click", function(e){
                                e.preventDefault();
                                var comment_id = $(this).attr('data-comment-id');
                                var callArgs = JSON.stringify([window.location.href, nascom_website_id, parseInt(comment_id)]);
                                nebPay.simulateCall(dappAddress, "0", "remove_comment", callArgs, {
                                    listener: function(resp) {
                                        nebPay.call(dappAddress, "0", "remove_comment", callArgs, {
                                        listener: function(resp) {
                                            console.log(resp);
...
$(document).ready(function() {
	var container = $("#nascom");
	var style = "<style>form{width: 400px;} #nascom-add-comment textarea {width:100%; display:block;margin:10px 0;background: none; border: 1px #dfdfdf solid; border-radius: 5px; padding: 5px;} input {background: none; border: 1px #dfdfdf solid; border-radius: 5px; padding: 5px;} input:hover {border: 1px grey solid; outline: none !important;} textarea:focus {outline: none !important; border: 1px grey solid;} .nascom-comment { border-bottom: 1px solid #eeeeee; padding: 10px 0; margin-bottom: 15px; } .nascom-comment span { display: block; color: #bfbfbf; font-size: 9pt; } .nascom-comment span.author { display: block; color: grey; font-size: 9pt; } #nascom-copyright {display:block; text-align: right; font-size: 8pt; color: #919191;} #nascom-copyright a { color: inherit; text-decoration: underline;} a.delete { float: right; color: white; background: #bcbcbc; border-radius: 13px; font-size: 12px; padding: 2px 8px; text-decoration: none; }</style>";
	var form = "<form id='nascom-add-comment'><textarea></textarea><span id='nascom-copyright'><a href='"+nascom_website+"profile.html' target='_blank'>Change nickname</a> | Powered by <a href='"+nascom_website+"' target='_blank'>Nascom</a></span><input type='submit' value='Comment'/></form>"
	container.append(style);
	container.append(form);
    form = $("#nascom-add-comment").on("submit", function(e){
        e.preventDefault();
        var text = $("#nascom-add-comment textarea").val();
        // console.log(text);
        add_comment(text);
    });

    var url = window.location.href;
    var callArgs = JSON.stringify([url]);
    nebPay.simulateCall(dappAddress, "0", "get_comments", callArgs, {
        listener: function(resp) {
            var result = JSON.parse(resp.result);

            if (result.success){
                console.log(result);
                result.comments = result.comments.sort(function(a, b){ return a > b;});

                result.comments.forEach(function(comment){
                    var callArgs = JSON.stringify([comment.author]);
                    nebPay.simulateCall(dappAddress, "0", "get_userinfo", callArgs, {
                        listener: function(resp) {
                            var result = JSON.parse(resp.result);
                            if(result){
                                render_comment(comment.text, comment.comment_id, result.name, comment.datetime);
                            }else{
                                render_comment(comment.text, comment.comment_id, comment.author, comment.datetime);                                
                            }
                        }
                    });
                });
