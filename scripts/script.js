...
$(document).ready(function() {
	$("#register-website").submit(function(e){
		e.preventDefault();
	...
	$("#profile").submit(function(e){
		e.preventDefault();
		var nickname = $("#profile input[name=nickname]").val();
		var email = $("#profile input[name=email]").val();
		var callArgs = JSON.stringify([nickname, email]);
		nebPay.simulateCall(dappAddress, "0", "save_userinfo", callArgs, {
		    listener: function(resp) {
		        var result = JSON.parse(resp.result);
				nebPay.call(dappAddress, "0", "save_userinfo", callArgs, {
				    listener: function(resp) {
				        var result = JSON.parse(resp.result);
				        console.log(result);
				    }
